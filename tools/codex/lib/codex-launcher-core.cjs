...
function launchCodex(mode, launcherDir, onExit, logger, onSpawn) {
    ...
    if (mode === "desktop") {
        // No pre-launch kill needed: config.toml's base_url is permanently
        // "http://127.0.0.1:53682/v1", written once by apply_codex and
        // never modified across model switches. Codex Desktop's in-memory
        // cache of config.toml is therefore always correct — switching
        // models in EchoBird only updates ~/.echobird/codex.json (read
        // fresh by the proxy per request), so Codex never needs to
        // re-read config or restart. UWP activation just brings the
        // existing window to front, which is the user's natural intent.
