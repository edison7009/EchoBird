...
interface ToolsState {
    ...
    setModelProtocolSelection: (sel: Record<string, 'openai' | 'anthropic'> | ((prev: Record<string, 'openai' | 'anthropic'>) => Record<string, 'openai' | 'anthropic'>)) => void;
...
export const useToolsStore = create<ToolsState>((set, get) => ({
    ...
    setDetectedTools: (tools) => set(state => ({
        detectedTools: typeof tools === 'function' ? tools(state.detectedTools) : tools,
    })),

    setModelProtocolSelection: (sel) => set(state => ({
