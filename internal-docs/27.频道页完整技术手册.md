...
```
用户选择频道 → SSH 连接成功 → 选择 Agent（AgentRolePicker）
  ↓
Agent 选中后触发：读取远程当前模型（SSH）
  ↓
UI 显示加载中（模型区域锁定）
  ↓
读取完成 → 显示当前远程模型信息
...
用户可选择本地模型列表中的模型替换
  ↓
点击确定 → SSH 写入远程配置 → 刷新显示
```

### UI 状态机

```
[Agent未选] ─── 选择Agent ──→ [读取远程模型中]
                                    │
                       ┌────────────┼────────────┐
                       ▼            ▼            ▼
              [读取成功]    [读取失败]    [无配置文件]
              显示模型名    显示错误      显示 "default"
                  │
                  ▼
           [用户选择新模型]
                  │
...
|------|------|------|
| 点击确定按钮 | ✅ | 最明确，用户有控制感 |
| 用户发消息时 | ❌ | 模型切换有延迟，影响发送体验 |
| 关闭界面时 | ❌ | 容易意外触发，失败时界面已关无法反馈 |

### 未选 Agent 时发消息

当用户未选择任何 Agent 时直接发消息，前端显示 `error.agentFailed` 系统消息。
不再静默失败（已修复：commit `7767545`）。
