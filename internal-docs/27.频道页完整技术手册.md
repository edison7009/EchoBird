...
Channels Page (React)
  ...
  │     └── 输入区 (flex-shrink-0) + 模型选择器
  └── AgentRolePicker (模态框 — 工具 + 角色选择)

通信方式 (本地与远程统一走 Bridge 协议):
  Local channel  → Bridge binary 已在本地，直接 spawn
  Remote channel → Bridge binary 通过 SSH 上传到远程服务器
  Protocol:        stdin/stdout JSON via Bridge binary (NOT WebSocket)

本地与远程的唯一区别:
  | 环节             | 本地                        | 远程                            |
  |-----------------|-----------------------------|--------------------------------|
  | Bridge 来源      | 本地已有，无需上传            | SSH 上传 + chmod + symlink      |
  | 工具检测        | detect_local_agents()       | bridge_detect_agents_remote()  |
  | 聊天             | bridge_chat_local()         | bridge_chat_remote() via SSH   |
  | 角色设置        | bridge_set_role_local()     | bridge_set_role_remote()       |
  | 模型读写        | bridge_get/set_local_model  | bridge_get/set_remote_model    |
  | 前端 UI / 流程  | 完全相同                      | 完全相同                         |
```
...
> [!IMPORTANT]
> 本地与远程走相同流程，仅传输层不同（本地 spawn vs 远程 SSH）。

```
发送
  ├─ Step 0: 检查模型是否已选 (未选 → error.noModelSelected)
  ├─ Step 1: 检测 Agent 安装状态
  │   ├─ 本地: detect_local_agents() [= scan_tools()]
  │   └─ 远程: bridgeDetectAgentsRemote (缓存)
  │        └─ agent 未安装 → error "Agent not installed"
  ├─ Step 2: 启动 Agent (如未运行)
  │   ├─ 本地: bridge_start(pluginId)
  │   └─ 远程: bridgeStartAgentRemote
  ├─ Step 3: 设置角色 (如有角色且变化)
  │   ├─ 本地: bridge_set_role_local
  │   └─ 远程: bridge_set_role_remote
  ├─ Step 4: 发送消息
  │   ├─ 本地: bridge_chat_local
  │   └─ 远程: bridge_chat_remote
  └─ 成功 → 显示 assistant 消息
```
...
> 本地和远程频道使用**完全相同的前端 UI 和流程**，仅后端传输层不同。
> App Manager 是独立页面，与 Channels 模型切换无关。
> 各 Agent 具体打通细节在 `29.远程模型切换设计方案.md` 中。

### 前置条件

| 条件 | 本地 | 远程 |
|------|------|------|
| 连接 | 本地 Bridge 已在 | SSH 已连接，服务器在线 |
| Agent | 在 AgentRolePicker 中选中已安装的 Agent | 同左 |

> [!IMPORTANT]
> **Local and remote channels both show the model selector**. Model switching uses the same unified flow:
> - Local: `bridge_get_local_model` / `bridge_set_local_model` (direct file I/O)
> - Remote: `bridge_get_remote_model` / `bridge_set_remote_model` (SSH)
> App Manager is an independent page with its own model switching logic.

### Bridge 自动部署策略（仅远程）

> [!NOTE]
> 本地频道 Bridge 已在本地，无需上传检测。以下仅用于远程频道。

远程操作都依赖 Bridge 二进制文件在远程服务器上。
采用**三层验证**策略，保证每个入口都有 Bridge：

| 时机 | 触发场景 | 代码位置 |
|------|---------|---------|
| 1. 添加服务器 | Mother Agent 首次 SSH 连接成功 | `ssh_commands.rs` |
| 2. 打开 Picker | 检测远程 Agent 安装状态 | `bridge_detect_agents_remote()` |
| 3. 发消息/设角色 | 远程聊天 / 远程设置角色 | `bridge_chat_remote()` / `bridge_set_role_remote()` |
...
| 本地频道 | 显示模型选择器（`bridge_get/set_local_model`，直接文件读写） |
...
- [ ] Bridge 三层验证 — `bridge_detect_agents_remote` 加 `ensure_remote_bridge` + Mother Agent 添加服务器加 `ensure_remote_bridge`
- [ ] 完成后清理 — 删除 Mother Agent 中关于 Bridge 部署和模型切换的提示词（本地 system prompt + 远程 prompt），这些功能已由 UI 接管
